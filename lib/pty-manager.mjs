 ...
 *
 * Replaces tmux for agent orchestration. Uses:
 *   - pty-bridge.py   real pseudo-terminal (python3, zero native deps)
 ...
 * this means spinners, progress bars, cursor movements, and TUI redraws
 * are handled correctly -- just like tmux capture-pane.
 *
 * tmux equivalents:
 *   tmux new-session    -> mgr.spawn(name, cmd)
 *   tmux send-keys      -> mgr.sendKeys(name, text)
 *   tmux capture-pane   -> mgr.capture(name, lines)
 *   tmux has-session    -> mgr.has(name)
 *   tmux kill-session   -> mgr.kill(name)
 *   tmux list-sessions  -> mgr.list()
 *   tmux display-message #{pane_pid} -> mgr.pid(name)
 *
 * Usage:
...
function findBridge() {
  // 1. same dir as this module (normal node execution)
  const candidates = [
    join(__dirname, "pty-bridge.py"),
  ];
  // 2. relative to the real binary (compiled binary: ../lib/pty-bridge.py)
  if (process.execPath) {
    const binDir = dirname(process.execPath);
    candidates.push(join(binDir, "lib", "pty-bridge.py"));
    candidates.push(join(binDir, "..", "lib", "pty-bridge.py"));
    candidates.push(join(binDir, "pty-bridge.py"));
  }
  // 3. PTY_BRIDGE env override
  if (process.env.PTY_BRIDGE) {
    candidates.unshift(process.env.PTY_BRIDGE);
  }
  for (const p of candidates) {
    if (existsSync(p)) return p;
  }
  throw new Error("pty-bridge.py not found. set PTY_BRIDGE env or place it next to the binary.");
}

const BRIDGE_PATH = findBridge();

class PtySession {
  constructor(name, bridgeProc, opts = {}) {
    this.name = name;
    this.proc = bridgeProc;
    this.bridgePid = bridgeProc.pid;
    this.childPid = null;
    ...
    // feed pty output into the terminal emulator
    ...
    // stderr = bridge control messages
    this.proc.stderr.on("data", (data) => {
      for (const line of data.toString().split("\n")) {
        if (line.startsWith("PID:")) {
          this.childPid = parseInt(line.slice(4), 10);
        } else if (line.startsWith("EXIT:")) {
          this.exitCode = parseInt(line.slice(5), 10);
        }
      }
    });
    ...
    this.proc.on("exit", (code, signal) => {
      ...
      this.events.emit("exit", { exitCode: this.exitCode, signal });
    });
   ...
   * this is the true equivalent of `tmux capture-pane -p`.
   *
  ...
  kill(signal = "SIGTERM") {
    if (!this.exited) {
      if (this.childPid) {
        try { process.kill(this.childPid, signal); } catch {}
      }
...
export class PtyManager {
  ...
  spawn(name, cmd, args = [], opts = {}) {
    ...
    const bridgeArgs = ["--size", `${cols}x${rows}`, cmd, ...args];
    const bridgeProc = spawn("python3", [BRIDGE_PATH, ...bridgeArgs], {
      cwd: opts.cwd || process.cwd(),
      env: { ...process.env, ...opts.env },
      stdio: ["pipe", "pipe", "pipe"],
    });

    const session = new PtySession(name, bridgeProc, {
      cwd: opts.cwd || process.cwd(),
      cmd: [cmd, ...args].join(" "),
      cols: opts.cols,
      rows: opts.rows,
  ...
  /**
   * capture rendered screen (tmux capture-pane equivalent).
...
const USAGE = `pty-mgr - PTY session manager (tmux replacement)
...
const _pat = /^(pty-manager(\.mjs)?|pty-mgr|p)$/;
